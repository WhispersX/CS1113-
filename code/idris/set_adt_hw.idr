...
clist_difference: (eq a) => list a -> list a -> list a
clist_difference l1 l2 = (list_difference l1 l2) ++ (list_difference l2 l1)

set_difference (mkSet l1) (mkSet l2) = mkSet (clist_difference l1 l2)
...
witness: (a-> bool) ->list a->option a
witness p nil = none
witness p (h::t) = ite (p h)
                       (some h)
                       (witness p t)

set_witness p (mkSet l) = witness p l
...
-- pair elt takes an elt h and a list b and returns list of all pairs,
...
--given a list a and alist of b return a list of all a-b pair
...
-- Here you need to provide an implementation of set_eql.
...
-- return true if s1 and s2 are equal, else false
set_eql s1 s2 = eql_set s1 s2

-- Return a string representation of s
--set_toString: (Serialize a) => set a -> String
toStringL: (Serialize a) => list a ->String
toStringL nil = "{}"
toStringL (h::nil) ="{"++ toString h++"}"++"{}"
toStringL (h::t) ="{"++ (toString h)++"}"++ "," ++ (toStringL t)


toStringSet: (Serialize a) => set a ->String
toStringSet (mkSet nil) = ""
toStringSet (mkSet l) = toStringL l

set_toString (mkSet l) = "{"++ toStringSet (mkSet l)++"}"
